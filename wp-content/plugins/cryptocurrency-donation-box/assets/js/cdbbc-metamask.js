...
async function cdbbc_change_network(chain_id, wallet, wallet_object) {
  ...
  if (wallet === "metamask_wallet" || wallet === "trust_wallet") {
    ...
    try {
      ...
      await chain_object.request({
        method: "wallet_switchEthereumChain",
        params: [{ chainId: nuumber_chain_id }],
      });

      /* Uncomment if you want to reload the page after switching the network
    ...
    // Handle the case where the wallet is not MetaMask or Trust Wallet
    Swal.close();
    Swal.fire({
