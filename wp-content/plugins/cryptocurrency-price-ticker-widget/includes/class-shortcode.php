...
if (!class_exists('CPTW_Shortcode')) {
    class CPTW_Shortcode
    {
        ...
        public function ccpw_shortcode($atts, $content = null)
        {
            ...
            $api = $this->ccpw_current_select_api();
            ...
            if($api == 'coin_paprika'){
                $api_by = 'Coinpaprika';
            }elseif($api == 'coin_marketcap'){
                $api_by = 'CoinMarketCap';
        ...
        /**
         * Generates HTML for widgets based on coin data.
         *
         * @param array $coin The coin data.
         * @param object $api_obj The API object for conversions.
         * @param string $fiat_currency The fiat currency code.
         * @param bool $is_cmc_enabled Flag to indicate if CoinMarketCap links are enabled.
         * @param string $type The type of widget.
         * @param bool $display_changes Flag to indicate if changes should be displayed.
         * @param array $usd_conversions The USD conversion rates.
         * @param string $cmc_slug The CoinMarketCap slug.
        ...
        protected function ccpw_widget_html($coin, $api_obj, $fiat_currency, $is_cmc_enabled, $type, $display_changes, $usd_conversions, $cmc_slug)
        {
            ...
            $api = isset($selected_api['ccpw_select_api']) && !empty($selected_api['ccpw_select_api']) ? sanitize_text_field($selected_api['ccpw_select_api']) : "coin_gecko";
            $coin_id = '';
            $coin_name = sanitize_text_field($coin['name']);
            $coin_symbol = sanitize_text_field($coin['symbol']);
            
            // Determine coin ID based on selected API
            if (in_array($api, ["coin_gecko", "coin_marketcap"])) {
                $coin_id = sanitize_text_field($coin['coin_id']);
            } else {
                $coin_id = sanitize_text_field($this->ccpw_coin_array($coin['coin_id']));
            }

            // Get coin logo HTML
            $coin_logo_html = $this->ccpw_get_coin_logo($coin_id, $coin_name, $coin['logo'], 32);
            if (!$coin_logo_html) {
                $coin_logo_html = '<img alt="' . esc_attr($coin_name) . '" src="' . esc_url(CCPWF_COINS_LOGO . sanitize_text_field($coin['logo'])) . '">';
            }

            // Process coin price
            $coin_price = isset($coin['price']) ? floatval($coin['price']) : 0;
            ...
            // Process percent change
            $percent_change_24h = number_format(floatval($coin['percent_change_24h']), 2, '.', ',') . '%';
            $change_sign = '<i class="dashicons dashicons-arrow-up" aria-hidden="true"></i>';
            $change_class = 'up';
            if (strpos($coin['percent_change_24h'], '-') !== false) {
                $change_sign = '<i class="dashicons dashicons-arrow-down" aria-hidden="true"></i>';
                $change_class = 'down';
            }

            // Generate coin link if CoinMarketCap is enabled
            $coin_link_start = '';
            $coin_link_end = '';
            if ($is_cmc_enabled) {
            ...
            switch ($type) {
                ...
                case 'multi-currency-tab':
                    ...
                    $EUR = isset($usd_conversions['EUR']) ? floatval($usd_conversions['EUR']) : 0.928516;
                    $GBP = isset($usd_conversions['GBP']) ? floatval($usd_conversions['GBP']) : 0.795123;
                    $AUD = isset($usd_conversions['AUD']) ? floatval($usd_conversions['AUD']) : 1.533634;
                    $JPY = isset($usd_conversions['JPY']) ? floatval($usd_conversions['JPY']) : 150.2420625;
                    ...
                    $coin_html .= $coin_link_end;
                    $coin_html .= '<div class="tab-price-area"><span data-aud="' . esc_attr($aud_price) . '" data-jpy="' . esc_attr($jpy_price) . '" data-gbp="' . esc_attr($gbp_price) . '" data-eur="' . esc_attr($euro_price) . '" data-usd="' . esc_attr($usd_price) . '" class="mtab_price">' . esc_html($usd_price) . '</span>';
                    if ($display_changes) {
    ...
    public function ccpw_get_ajax_data()
    {
         ...
         * @param string $back_color The background color.
         * @param string $font_color The font color.
         * @param string $ticker_top The top position for the ticker.
        ...
        public function ccpw_dynamic_style($type, $post_id, $back_color, $font_color, $ticker_top)
        {
            ...
            $post_id = (int) $post_id;
            $back_color = sanitize_hex_color($back_color);
            $font_color = sanitize_hex_color($font_color);
            $ticker_top = sanitize_text_field($ticker_top);

            $dynamic_styles = "";

            $bg_color = !empty($back_color) ? 'background-color:' . esc_attr($back_color) . ';' : 'background-color:#fff;';
            $tbl_bg_color = !empty($back_color) ? 'background-color:' . esc_attr($back_color) . ' !important;' : '';
            $fnt_color = !empty($font_color) ? 'color:' . esc_attr($font_color) . ';' : 'color:#000;';
            $tbl_fnt_color = !empty($font_color) ? 'color:' . esc_attr($font_color) . ';' : '';
            ...
            switch ($type) {
                case 'ticker':
                    ...
                    $dynamic_styles .= '.tickercontainer #' . esc_attr($id) . '{' . $bg_color . '}
                        .tickercontainer #' . esc_attr($id) . ' span.name,
                        .tickercontainer #' . esc_attr($id) . ' .ccpw-credits a {' . $fnt_color . '}
                        .tickercontainer #' . esc_attr($id) . ' span.coin_symbol {' . $fnt_color . '}
                        .tickercontainer #' . esc_attr($id) . ' span.price,
                        .tickercontainer .price-value{' . $fnt_color . '}
                        .ccpw-header-ticker-fixedbar{' . esc_attr($ticker_top) . '}';
                    break;
                case 'price-label':
                    // Handling price label widget
                    $id = 'ccpw-label-widget-' . $post_id;
                    $dynamic_styles .= '#' . esc_attr($id) . '.ccpw-price-label li a , #' . esc_attr($id) . '.ccpw-price-label li{' . $fnt_color . '}';
                    break;
                case 'list-widget':
                    // Handling list widget
                    $id = 'ccpw-list-widget-' . $post_id;
                    $dynamic_styles .= '#' . esc_attr($id) . '.ccpw-widget .ccpw_table tr{' . $bg_color . '}
                        #' . esc_attr($id) . '.ccpw-widget .ccpw_table tr th, #' . esc_attr($id) . '.ccpw-widget .ccpw_table tr td,
                        #' . esc_attr($id) . '.ccpw-widget .ccpw_table tr td a{' . $fnt_color . '}';
                    break;
                case 'multi-currency-tab':
                    // Handling multi-currency tab widget
                    $id = 'ccpw-multicurrency-widget-' . $post_id;
                    $dynamic_styles .= '.currency_tabs#' . esc_attr($id) . ',.currency_tabs#' . esc_attr($id) . ' ul.multi-currency-tab li.active-tab{' . $bg_color . '}
                    .currency_tabs#' . esc_attr($id) . ' .mtab-content, .currency_tabs#' . esc_attr($id) . ' ul.multi-currency-tab li, .currency_tabs#' . esc_attr($id) . ' .mtab-content a{' . $fnt_color . '}';
                    break;
                case 'table-widget':
                    // Handling table widget
                    $id = 'ccpw-datatable-' . $post_id;
                    if ($tbl_bg_color || $tbl_fnt_color) {
                        $dynamic_styles .= '.ccpw-coinslist_wrapper #' . esc_attr($id) . ' tr td,.ccpw-coinslist_wrapper #' . esc_attr($id) . ' tr th,.ccpw-coinslist_wrapper .dataTables_paginate a {' . $tbl_bg_color . $tbl_fnt_color . '}';
                    }
                    break;
            }
