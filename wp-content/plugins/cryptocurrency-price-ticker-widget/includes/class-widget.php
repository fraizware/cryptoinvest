...
class ccpw_Widget extends WP_Widget
{
    ...
    public function __construct()
    {
        $widget_options = array(
            'classname' => 'ccpw_widget',
            'description' => 'Cryptocurrency Widgets by Cool Plugins',
        );
  ...
  <p>
	...
	<input type="text" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" />
	  ...
	  <p>
		<label for="<?php echo esc_attr($this->get_field_id('ccpw_shortcode')); ?>"> Shortcode:</label>
		 <select style="width:70%" id="<?php echo esc_attr($this->get_field_id('ccpw_shortcode')); ?>" name="<?php echo esc_attr($this->get_field_name('ccpw_shortcode')); ?>" >
        ...
        if ($postlist) {
            foreach ($postlist as $post):
                ...
                if ($ccpw_shortcode_id == $p_id) {
                    echo '<option selected="selected" value="' . esc_attr($p_id) . '">[ccpw id="' . esc_html($p_id) . '"]' . '</option>';
                } else {
                    echo '<option value="' . esc_attr($p_id) . '">[ccpw id="' . esc_html($p_id) . '"]' . '</option>';
                }
